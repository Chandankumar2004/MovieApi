document.addEventListener("DOMContentLoaded", () => {
  ...
  function addEventListeners() {
    ...
    elements.sortOrder.addEventListener("change", () => renderMovies(currentMovies));
  ...
  async function loadMovies(query, page) {
    ...
    try {
      ...
      elements.moviesContainer.innerHTML = "<p>Something went wrong. Please try again later.</p>";
    } finally {
  ...
  function renderMovies(movies) {
    elements.moviesContainer.innerHTML = "";
    const moviesWithPosters = movies.filter(movie => movie.Poster && movie.Poster.trim() !== "" && movie.Poster.toLowerCase() !== "n/a");
    ...
    sortedMovies.forEach((movie) => {
      ...
      card.innerHTML = `
        ...
        <button class="fav-btn ${isFav ? 'favorited' : ''}" data-id="${movie.imdbID}">♥</button>
        <div class="info">
          <h3>${movie.Title} (${movie.Year})</h3>
          <button class="details-btn" data-id="${movie.imdbID}">Show Details</button>
        </div>
      `;
  ...
  function handleSurpriseMe() {
    const randomQuery = surpriseMeQueries[Math.floor(Math.random() * surpriseMeQueries.length)];
  ...
  async function showDetails(id) {
    ...
    try {
      ...
      elements.modalBody.innerHTML = `
        ...
        <p>${movie.Plot}</p>
        <img src="${movie.Poster !== "N/A" ? movie.Poster : 'https://via.placeholder.com/200x300.png?text=No+Image'}" style="width:100%;border-radius:10px;">
        <button id="shareBtn" data-id="${movie.imdbID}">Share</button>
      `;
      document.getElementById("shareBtn").addEventListener("click", (e) => {
        navigator.clipboard.writeText(`https://www.imdb.com/title/${e.target.dataset.id}/`);
  ...
  function saveTheme() { localStorage.setItem("theme", isDarkMode ? "dark" : "light"); }
  function loadTheme() {
  ...
  function toggleView() {
    ...
    elements.viewToggleBtn.textContent = currentView === "grid" ? "List View" : "Grid View";
    saveView();
  }

  function saveView() { localStorage.setItem("view", currentView); }
  function loadView() {
  ...
  function loadFavorites() { favorites = JSON.parse(localStorage.getItem("favorites")) || []; }

  async function displayFavorites() {
    elements.moviesContainer.innerHTML = "";
    elements.pagination.innerHTML = "";
    if (favorites.length === 0) {
      elements.moviesContainer.innerHTML = "<p>You have no favorite movies yet.</p>";
      return;
    }
    elements.loader.style.display = "block";
  ...
  function saveRecentSearches() { localStorage.setItem("recentSearches", JSON.stringify(recentSearches)); }
  ...
  function updatePagination() {
    ...
    if (totalPages > 1) {
      ...
      if (currentPage < totalPages) html += `<button id="nextBtn">Next</button>`;
      elements.pagination.innerHTML = html;
    }
  }

  function handleScroll() {
    elements.backToTopBtn.style.display = window.scrollY > 200 ? "block" : "none";
  }
